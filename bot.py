...
AI_MUTE_PROTOCOL = (
    ...
    "Эта пометка нужна тебе ТОЛЬКО чтобы правильно указывать цель в командах мьюта/размута — "
    "не копируй её в текст своего обычного ответа и не начинай ответ с обращения по нику "
    "(\"@username, ...\"), если это не требуется по смыслу разговора (например, специально "
    "выделяешь кого-то). В большинстве случаев отвечай без упоминания ника вообще, обычной "
    "живой репликой — Telegram и так покажет, что это ответ именно ему, отдельно называть его "
    "не нужно.\n"
